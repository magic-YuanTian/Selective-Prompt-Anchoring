...
class SPALogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, aux_model, aux_input_ids, mask_token, strength=1.4, modulated_by_prob=True, tokenizer=None, use_attention_mask=True):
...
if __name__ == "__main__":
    ...
    # --- Hyperparameters ---
    anchoring_strength = 0 # Weight towards the modified prompt's influence
    modulated_by_prob = False  # If True, the anchoring strength is weighted by token probability. Turn it on for stable results.
    ...
    # Configure quantization options with single device instead of auto mapping
    model_kwargs = {
        # Configure the Huggingface model here
        "device_map": device
    }
    
    # Add quantization only for CUDA devices
    if device == "cuda":
    ...
    # global_anchors = ['basketball']
    global_anchors = ['introverted']

    # Usage 1
    # prompt_with_anchors = "Tom is good at piano and basketball. What gift should I give to him for his birthday? Directly and only answer 1 gift name."
    prompt_with_anchors = "I am a introverted person. How to describe my personality? Answer in 1 sentence."

    # Usage 2
    prompt_with_anchors = "What's the weather <anchor>today</anchor>? Think <anchor>step by step</anchor>."
    ...
    # Create a default generation config if none exists
